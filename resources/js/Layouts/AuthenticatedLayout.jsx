            ...
            <nav className="bg-white border-b border-gray-100">
                        ...
                        <div className="flex">
                            ...
                            <div className="hidden space-x-6 sm:-my-px sm:ms-10 lg:flex">
                                <NavLink href={route('dashboard')} active={route().current('dashboard')}>
                                    ...
                                    {user.role === 'super_admin' ? t('fermes') : t('dashboard')}
                                </NavLink>
                                ...
                                <NavLink href={route('harvests.index')} active={route().current('harvests.index')}>
                                    <svg className="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M12 3v1m0 16v1m9-9h-1M4 12H3m15.364-6.364l-.707.707M6.343 17.657l-.707.707m0-12.728l.707.707m12.728 12.728l-.707.707M12 8a4 4 0 100 8 4 4 0 000-8z" /></svg>
                                    {t('harvests') || 'Récoltes'}
                                </NavLink>
                                ...
                                <div className="hidden sm:flex sm:items-center">
                                    <Dropdown>
                                        <Dropdown.Trigger>
                                            <button type="button" className="inline-flex items-center px-1 pt-1 border-b-2 border-transparent text-sm font-black uppercase tracking-tighter text-gray-500 hover:text-gray-700 hover:border-gray-300 focus:outline-none transition duration-150 ease-in-out h-16">
                                        ...
                                        <Dropdown.Content width="64">
                                            <Dropdown.Link href={route('analytics.index')} className="flex items-center">
                                            ...
                                            <Dropdown.Link href={route('payroll.history')} className="flex items-center">
                                            ...
                                            <Dropdown.Link href={route('settings.index')} className="flex items-center border-t border-gray-50 mt-1 pt-2">
                                        ...
                                        <span className="inline-flex rounded-md">
                                            ...
                                            >
                                                <div className="flex flex-col items-start mr-3 border-r pr-3 border-gray-100 hidden md:flex">
                                                    <span className="text-[10px] font-black text-blue-600 uppercase tracking-widest leading-none">{user.farm?.name || 'Globale'}</span>
                ...
                <div className={(showingNavigationDropdown ? 'block' : 'hidden') + ' lg:hidden bg-white border-t border-gray-100 shadow-2xl'}>
                    <div className="pt-2 pb-3 space-y-1">
                        <ResponsiveNavLink href={route('dashboard')} active={route().current('dashboard')} onClick={() => setShowingNavigationDropdown(false)}>
                            <div className="flex items-center">
                                ...
                                {user.role === 'super_admin' ? 'Fermes' : 'Dashboard'}
                            </div>
                        </ResponsiveNavLink>

                        <ResponsiveNavLink href={route('pointage.index')} active={route().current('pointage.index')} onClick={() => setShowingNavigationDropdown(false)}>
                            <div className="flex items-center">
                        ...
                        <ResponsiveNavLink href={route('employees.index')} active={route().current('employees.index')} onClick={() => setShowingNavigationDropdown(false)}>
                            <div className="flex items-center">
                        ...
                        <ResponsiveNavLink href={route('harvests.index')} active={route().current('harvests.index')} onClick={() => setShowingNavigationDropdown(false)}>
                            <div className="flex items-center">
                                <svg className="w-5 h-5 mr-3 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M12 3v1m0 16v1m9-9h-1M4 12H3m15.364-6.364l-.707.707M6.343 17.657l-.707.707m0-12.728l.707.707m12.728 12.728l-.707.707M12 8a4 4 0 100 8 4 4 0 000-8z" /></svg>
                                Récoltes
                            </div>
                        </ResponsiveNavLink>

                        {user.role === 'super_admin' && (
                            <ResponsiveNavLink href={route('users.index')} active={route().current('users.index')} onClick={() => setShowingNavigationDropdown(false)}>
                                <div className="flex items-center">
                        ...
                        <div className="border-t border-gray-100 my-2 pt-2">
                            <ResponsiveNavLink href={route('analytics.index')} active={route().current('analytics.index')} onClick={() => setShowingNavigationDropdown(false)}>
                            ...
                            <ResponsiveNavLink href={route('payroll.history')} active={route().current('payroll.history')} onClick={() => setShowingNavigationDropdown(false)}>
                            ...
                            <ResponsiveNavLink href={route('settings.index')} active={route().current('settings.index')} onClick={() => setShowingNavigationDropdown(false)}>
