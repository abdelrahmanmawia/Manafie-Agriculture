...
There is no Laravel Policy/Gate layer and no role middleware registered in `app/Http/Kernel.php`. Access control is done by hand by comparing `$request->user()->role` against string literals: `'super_admin'`, `'farm_manager'`, `'data_entry'`. Two different patterns exist, split by domain:
- **Pointage/Analytics/Payroll** controllers inline the branching per-method (see [PointageController.php](app/Http/Controllers/PointageController.php)): `super_admin` sees everything and can pass an explicit `enterprise_id`/`farm_id` query param; `farm_manager` is scoped to their `farm_id` (via `enterprise.farm_id`); everyone else is scoped to their own `enterprise_id`, falling back to `farm_id` if unset. Cross-tenant access returns `abort(403)`, not a 404.
- **Stock** controllers use two shared helpers on the base [Controller.php](app/Http/Controllers/Controller.php): `scopedFarmId()` for reads (super_admin gets `?farm_id=` as an optional filter, defaulting to "all farms"; everyone else locked to their own `farm_id`) and `resolveWriteFarmId()` for writes (super_admin must supply `farm_id` in the request body; everyone else forced to their own farm). Stock has no enterprise concept — it's pooled at the farm level, shared across all of a farm's enterprises/divisions.
